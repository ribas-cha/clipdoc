...
def extract_hepatograma_pancreas(lines):
    ...
    # Correção: Iterar sobre `lines` para checar o contexto, não `line_chunk` que não está definido aqui.
    ...
    if is_hepatograma_context:
        ...
        results["TGO"] = tgo_val
        results["TGP"] = tgp_val
        if not results["TGO"]: results["TGO"] = extract_labeled_value(lines,
                                                                      ["TGO", "AST", "Aspartato amino transferase"],
                                                                      label_must_be_at_start=False,
                                                                      search_window_lines=1, require_unit="U/L")
        if not results["TGP"]: results["TGP"] = extract_labeled_value(lines,
                                                                      ["TGP", "ALT", "Alanina amino transferase"],
                                                                      label_must_be_at_start=False,
                                                                      search_window_lines=1, require_unit="U/L")
...
def evoluir_paciente_enfermaria_ia_fase2(resumo_ia_fase1, dados_medico_hoje, evolucao_anterior_original):
    ...
    campos_a_manter = ["#CUIDADOS PALIATIVOS:", "#ID:", "#HD:", "#AP:", "#HDA:", "#MUC:", "#ALERGIAS:", "#ATB:",
                       "#TEV:"]

    # Para o campo #EXAMES, precisamos de uma lógica mais elaborada para pegar o bloco inteiro
    exames_bloco_anterior = []
    capturando_exames = False
    for linha in linhas_evol_anterior:
        if linha.strip().startswith("#EXAMES:"):
            capturando_exames = True
            exames_bloco_anterior.append(linha)  # Adiciona a linha do header
            continue
        if capturando_exames:
            if linha.strip().startswith("#"):  # Encontrou o próximo header, para de capturar
                capturando_exames = False
            else:
                exames_bloco_anterior.append(linha)

    # Extrai outros campos fixos
    for linha in linhas_evol_anterior:
        for campo_chave in campos_a_manter:
            if linha.strip().startswith(campo_chave):
                # Pega o conteúdo após a chave até o final da linha ou até o próximo #
                conteudo_campo = linha.split(campo_chave, 1)[-1].strip()
                campos_fixos[campo_chave] = conteudo_campo
                break  # Vai para a próxima linha do arquivo

    # Monta a parte inicial do template com os dados extraídos
    template_parte_fixa = f"""# UNIDADE DE INTERNAÇÃO - EVOLUÇÃO#

#CUIDADOS PALIATIVOS: {campos_fixos.get("#CUIDADOS PALIATIVOS:", "")}
#ID: {campos_fixos.get("#ID:", "")}
#HD: {campos_fixos.get("#HD:", "")}
#AP: {campos_fixos.get("#AP:", "")}
#HDA: {campos_fixos.get("#HDA:", "")}
#MUC: {campos_fixos.get("#MUC:", "")}
#ALERGIAS: {campos_fixos.get("#ALERGIAS:", "")}
#ATB: {campos_fixos.get("#ATB:", "")}
#TEV: {campos_fixos.get("#TEV:", "")}
"""
    # Adiciona o bloco de exames anterior
    if exames_bloco_anterior:
        template_parte_fixa += "\n".join(exames_bloco_anterior) + "\n"
    else:  # Se não havia bloco de exames, mantém o template original
        template_parte_fixa += """
#EXAMES:
>CULTURAS/ANTÍGENOS:
>IMAGEM:
...
"""
    ...
    # Template para as partes que a IA deve gerar/modificar
    template_parte_variavel = """
#EVOLUÇÃO (Narrativa do dia):
[IA: Crie uma nova narrativa para HOJE baseada nos 'Novos dados e observações do médico'. Integre de forma coesa com o contexto do paciente da 'Análise da IA sobre a evolução anterior'.]

#EXAME FÍSICO (Detalhado do dia):
[IA: Comece com o exame físico da evolução anterior (se disponível na 'Análise da IA'). Incorpore e priorize as NOVAS ALTERAÇÕES e achados do exame físico fornecidos em 'Novos dados e observações do médico para a evolução de HOJE'. Formate cada item do exame físico começando com um hífen ('- '). Se o médico fornecer um exame físico completo para hoje, use-o predominantemente.]

#PLANO TERAPÊUTICO (Atualizado para hoje):
[IA: Formule com base na 'Análise da IA', nas 'Sugestões de conduta da IA' e, crucialmente, nos 'Novos dados e observações do médico para a evolução de HOJE'.]

#CONDUTA (Para hoje):
[IA: Formule em formato de ITENS COM HÍFEN, baseada em TODAS as informações: análise da IA, sugestões da IA e novos dados do médico.]
...
MANTENHA OS SEGUINTES CAMPOS EXATAMENTE COMO ESTÃO NA 'Evolução Anterior Original', A MENOS QUE HAJA INFORMAÇÃO CONTRADITÓRIA DIRETA NOS 'Novos dados e observações do médico para a evolução de HOJE':
...
FOQUE A NOVA GERAÇÃO DE TEXTO E MODIFICAÇÕES PRINCIPALMENTE NOS SEGUINTES CAMPOS, utilizando a 'Análise da IA sobre a evolução anterior' e os 'Novos dados e observações do médico para a evolução de HOJE':
- #EVOLUÇÃO (Narrativa do dia): Crie uma nova narrativa para hoje.
- #EXAME FÍSICO (Detalhado do dia): Integre os novos achados do médico ao exame físico anterior. Formate cada item com um hífen ('- ').
- #PLANO TERAPÊUTICO (Atualizado para hoje): Formule o plano.
- #CONDUTA (Para hoje): Formule em formato de ITENS COM HÍFEN.
- #DATA PROVÁVEL DA ALTA: Atualize se houver nova informação.

(1) Análise da IA sobre a evolução anterior (Resumo do caso, Pontos de discussão, Exame físico a avaliar, Sugestões de conduta):
---
{resumo_ia_fase1}
---

(2) Evolução Anterior Original (Use esta para os campos que devem ser mantidos):
---
{evolucao_anterior_original}
---

(3) Novos dados e observações do médico para a evolução de HOJE (anamnese, exame físico, resultados de exames, etc.):
---
{dados_medico_hoje}
---

Gere a nota de EVOLUÇÃO MÉDICA para HOJE, utilizando o template abaixo como guia. Preencha os campos conforme as instruções, priorizando a manutenção dos dados da evolução anterior para os campos designados e integrando as novas informações onde apropriado.
...
def gerar_resumo_alta_ia(ultima_evolucao):
    prompt = f"""Você é um médico hospitalista experiente. Com base na última evolução do paciente fornecida abaixo, redija um resumo de alta hospitalar conciso e claro, em dois ou três parágrafos.
...
Use linguagem clara e direta, evitando jargões médicos excessivos.

Última Evolução:
---
{ultima_evolucao}
---
Resumo de Alta (2-3 parágrafos):
"""
    return gerar_resposta_ia(prompt)
...
Com base no caso do paciente descrito abaixo, gere orientações de alta claras e diretas para o paciente e seus familiares.
As orientações devem focar em sinais e sintomas de alerta que indicariam a necessidade de retornar ao Pronto-Socorro.
...
if not GOOGLE_API_KEY and api_key_source != "secrets":
    st.warning(
        "Chave da API do Google não configurada para desenvolvimento local. Funcionalidades de IA estarão desabilitadas. Defina-a na variável `GOOGLE_API_KEY_LOCAL_FALLBACK` no código ou como variável de ambiente `GOOGLE_API_KEY`.")
elif GOOGLE_API_KEY and not gemini_available:
    st.error(st.session_state.get("gemini_config_error",
                                  "Erro desconhecido ao configurar a API do Gemini. Funcionalidades de IA desabilitadas."))
...
with tab1:
    ...
    if "show_compatible_exams_detailed_tab1" not in st.session_state: st.session_state[
    ...
    with col1_tab1:
        ...
        if action_cols_tab1[0].button("🔍 Analisar Exame", use_container_width=True, type="primary",
                                      key="btn_analisar_exame_tab1"):
        ...
        if action_cols_tab1[2].button("📋 Exames Compatíveis", use_container_width=True, key="btn_compat_tab1"):
            st.session_state["show_compatible_exams_detailed_tab1"] = not st.session_state[
                "show_compatible_exams_detailed_tab1"]
    ...
    with col2_tab1:
        ...
        st.text_area("Resultados formatados:", value=st.session_state.get("saida_exames", ""), height=350,
                     key="saida_text_main_display_tab1", label_visibility="collapsed", disabled=True)
...
with tab2:  # Aba do Agente IA
    ...
    if not gemini_available:
        st.error(
            "O Agente IA Hospitalista está indisponível no momento. Verifique a configuração da chave API ou o erro de configuração acima.")
    else:
        ia_task_options = [
        ...
        tarefa_ia_selecionada = st.selectbox("Qual tarefa o Agente IA deve realizar?", ia_task_options,
        ...
        if tarefa_ia_selecionada == "Evoluir Paciente (Enfermaria - Interativo)":
            ...
            if st.session_state.ia_fase_evolucao_interativa == 1:
                evolucao_anterior_input_ia = st.text_area("1. Cole a evolução do dia ANTERIOR aqui:", height=200,
                ...
                with col_btn1:
                    if st.button("Gerar Evolução Final com IA", key="btn_ia_evol_enf_fase2"):
                        ...
                        if dados_medico_hoje_input:
                            ...
                            st.warning("Por favor, adicione seus achados de hoje.")
                with col_btn2:
            ...
            if st.session_state.ia_fase_evolucao_interativa == 3:
                if st.session_state.ia_output_evolucao_enf_fase1: st.markdown("---"); st.markdown(
                    "**Análise e Sugestões da IA (baseado na evolução anterior):**"); st.markdown(
                    st.session_state.ia_output_evolucao_enf_fase1)
                if st.session_state.ia_dados_medico_hoje: st.markdown("---"); st.markdown(
                    "**Seus achados de HOJE (fornecidos à IA):**"); st.markdown(st.session_state.ia_dados_medico_hoje)
                if st.session_state.ia_output_evolucao_final:
                    st.markdown("---");
                    st.subheader("Evolução Médica Final (Gerada pela IA):")
                    st.text_area("Evolução:", value=st.session_state.ia_output_evolucao_final, height=400,
                                 key="ia_evolucao_final_display", disabled=True)
                    components.html(
                        f"""<textarea id="cClipEvolFinal" style="opacity:0;position:absolute;left:-9999px;top:-9999px;">{st.session_state.ia_output_evolucao_final.replace("'", "&apos;").replace('"', '&quot;')}</textarea><button onclick="var t=document.getElementById('cClipEvolFinal');t.select();t.setSelectionRange(0,99999);try{{var s=document.execCommand('copy');var m=document.createElement('div');m.textContent=s?'Evolução copiada!':'Falha.';m.style.cssText='position:fixed;bottom:20px;left:50%;transform:translateX(-50%);padding:10px 20px;background-color:'+(s?'#28a745':'#dc3545')+';color:white;border-radius:5px;z-index:1000;';document.body.appendChild(m);setTimeout(function(){{document.body.removeChild(m);}},2000);}}catch(e){{alert('Não foi possível copiar.');}}" style="padding:10px 15px;background-color:#28a745;color:white;border:none;border-radius:5px;cursor:pointer;width:100%;margin-top:10px;">📋 Copiar Evolução Final</button>""",
                        height=65)
            ...
            info_caso_input_ia = st.text_area("Forneça as informações do caso para admissão:", height=300,
                                              key="ia_adm_info_input_tab2")
            if st.button("Gerar Admissão com IA", key="btn_ia_adm_tab2"):
                if info_caso_input_ia:
                    ...
                    st.warning("Por favor, forneça as informações do caso.")
            if st.session_state.ia_output_admissao:
                st.markdown("---");
                st.subheader("Rascunho da Admissão (gerado pela IA):")
                st.text_area("Modelo Preenchido:", value=st.session_state.ia_output_admissao, height=400,
                             key="ia_admissao_output_display_tab2", disabled=True)
                components.html(
                    f"""<textarea id="cClipAdmissaoTab2" style="opacity:0;position:absolute;left:-9999px;top:-9999px;">{st.session_state['ia_output_admissao'].replace("'", "&apos;").replace('"', "&quot;")}</textarea><button onclick="var t=document.getElementById('cClipAdmissaoTab2');t.select();t.setSelectionRange(0,99999);try{{var s=document.execCommand('copy');var m=document.createElement('div');m.textContent=s?'Admissão copiada!':'Falha.';m.style.cssText='position:fixed;bottom:20px;left:50%;transform:translateX(-50%);padding:10px 20px;background-color:'+(s?'#28a745':'#dc3545')+';color:white;border-radius:5px;z-index:1000;';document.body.appendChild(m);setTimeout(function(){{document.body.removeChild(m);}},2000);}}catch(e){{alert('Não foi possível copiar.');}}" style="padding:10px 15px;background-color:#28a745;color:white;border:none;border-radius:5px;cursor:pointer;width:100%;margin-top:10px;">📋 Copiar Rascunho da Admissão</button>""",
                    height=65)
            ...
            ultima_evolucao_input_alta = st.text_area("Cole a ÚLTIMA evolução completa do paciente aqui:", height=300,
                                                      key="ia_input_resumo_alta")
